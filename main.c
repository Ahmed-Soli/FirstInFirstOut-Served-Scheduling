...
int main()
{
    ...
    int wt[np];
    int pid[np]; // array is a pointer which is pointing to the first element of the array arr[0]
...
void setNumProcess(int *np)
{
    ...
    {
        *np =atoi(nps); //atio turns the string into an integer
    }
};
...
void displayProcessDetail(int np,float *sum,Queue *prq,int *wt,int *pid)
{
    ...
    printf("---------------\n");
    int i = 0;
    wt[0]=0; // first process starts at once without waiting time
    while(!QueueEmpty(prq))
    {
        Serve(&p,prq); // prq is a pointer because it was passed as a pointer 
        // equal to p = server(prq);
        printf("%d\t | %d\n",p.pid,wt[i]);
        *sum+=wt[i]; // sum = sum + wt[i] // * pointer to sum which means edit this value
        pid[i]=p.pid;
        i++;
        wt[i]=p.burst+wt[i-1]; // equal to wt[i+1]=p.burst+wt[i]; then i++;
    }
    printf("---------------\n");
};
